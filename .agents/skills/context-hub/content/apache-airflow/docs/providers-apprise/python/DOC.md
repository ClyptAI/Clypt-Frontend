...
```bash
export AIRFLOW_CONN_APPRISE_DEFAULT='{"extra": {"config": {"path": "https://hooks.slack.example.invalid/services/TEAM/CHANNEL/WEBHOOK_TOKEN", "tag": "ops"}}}'
```

Multiple-service example:

```bash
export AIRFLOW_CONN_APPRISE_DEFAULT='{"extra": {"config": [{"path": "https://hooks.slack.example.invalid/services/TEAM/CHANNEL/WEBHOOK_TOKEN", "tag": "ops"}, {"path": "mailto://user:password@example.com", "tag": "email"}]}}'
```
