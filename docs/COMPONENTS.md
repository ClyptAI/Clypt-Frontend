...
| `Hero` | `Hero.tsx` | Landing hero with a violet `GemSmoke` `ShaderBackground`, animated two-line headline, paste-link CTA bar, `Try free now` signup link, `See demo` link into `/runs/demo/timeline`, and the right-side `ClyptHeroAnimation`. |
| `ClyptHeroAnimation` | `ClyptHeroAnimation.tsx` | First-viewport hero animation. Runs once per page load through video analysis, semantic graph reveal, clip fanout, and a settled `Top Hit` center card. Uses tracked poster assets in `public/images/landing-posters/`, keeps the fanout score chips in the top-right of each clip card, and respects reduced motion by jumping to the settled state. |
...
| `PipelineDemos` | `PipelineDemos.tsx` | Sticky scrollytelling orchestrator for the six landing phase sections. A left-side progress rail tracks the active phase while the right pane swaps app-frame previews. Supports per-phase right bleed for wide previews, including the cortex graph stage, so preview nodes do not clip at the card edge. Outer copy stays general-marketing; embedded previews carry the Joe Rogan x Flagrant demo internals. |
