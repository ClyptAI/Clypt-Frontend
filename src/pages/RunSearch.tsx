import { useState, useCallback } from "react";
import { useParams } from "react-router-dom";
import { Sparkles } from "lucide-react";
import RunContextBar from "@/components/app/RunContextBar";
import { EmbedScatter, EmbedInspectPanel, SearchBar, SearchResultsPanel, PANEL_H } from "@/components/embeds";
import type { EmbedType, ScoredPoint } from "@/components/embeds";
import { useEmbeddings } from "@/hooks/api/useEmbeddings";
import type { EmbedPoint } from "@/hooks/api/useEmbeddings";
import { useRunDetail } from "@/hooks/api/useRuns";

// ── Legend ────────────────────────────────────────────────────────────────────

const LEGEND_TYPES: Array<{ type: string; color: string }> = [
  { type: "claim",             color: "#A78BFA" },
  { type: "anecdote",          color: "#FBB249" },
  { type: "reveal",            color: "#FACC15" },
  { type: "qa_exchange",       color: "#38BDF8" },
  { type: "challenge_exchange",color: "#FB923C" },
  { type: "explanation",       color: "#60A5FA" },
  { type: "reaction_beat",     color: "#4ADE80" },
  { type: "setup_payoff",      color: "#FB923C" },
  { type: "hook",              color: "#4ADE80" },
  { type: "insight",           color: "#A78BFA" },
  { type: "conflict",          color: "#EF4444" },
  { type: "transition",        color: "#71717A" },
];

// ── Mock semantic search ──────────────────────────────────────────────────────
// Scores by word overlap in summary + transcript_excerpt; always returns
// at least 4 results (fills remainder with hash-ranked nodes) so the UI
// feels alive even for non-English / nonsense queries.

function mockSearch(query: string, points: EmbedPoint[]): ScoredPoint[] {
  if (!points.length) return [];
  const terms = query
    .toLowerCase()
    .replace(/[¿?¡!.,;:]/g, " ")
    .split(/\s+/)
    .filter((t) => t.length > 2);

  const scored = points.map((p) => {
    const haystack =
      `${p.node_type} ${p.summary} ${p.transcript_excerpt ?? ""}`.toLowerCase();
    let hits = 0;
    for (const t of terms) {
      if (haystack.includes(t)) hits++;
    }
    const score = terms.length ? hits / terms.length : 0;
    return { ...p, score };
  });

  const matched = scored.filter((p) => p.score > 0).sort((a, b) => b.score - a.score);

  // If fewer than 4 keyword matches, fill with hash-ranked results
  if (matched.length < 4) {
    const queryHash = query
      .split("")
      .reduce((acc, c) => (acc * 31 + c.charCodeAt(0)) & 0xffffffff, 0);
    const usedIds = new Set(matched.map((p) => p.node_id));
    const filler = scored
      .filter((p) => !usedIds.has(p.node_id))
      .sort((a, b) => {
        const ha = (queryHash ^ a.node_id.charCodeAt(a.node_id.length - 1)) & 0xff;
        const hb = (queryHash ^ b.node_id.charCodeAt(b.node_id.length - 1)) & 0xff;
        return hb - ha;
      })
      .slice(0, 5 - matched.length)
      .map((p, i) => ({ ...p, score: 0.45 - i * 0.04 }));
    return [...matched, ...filler].slice(0, 8);
  }

  return matched.slice(0, 8);
}

// ── Page ─────────────────────────────────────────────────────────────────────

export default function RunSearch() {
  const { id: runId = "" } = useParams<{ id: string }>();
  const { data: runDetail } = useRunDetail(runId);
  const { data: embedData, isLoading } = useEmbeddings(runId);

  const [embedType, setEmbedType] = useState<EmbedType>("semantic");
  const [selectedNode, setSelectedNode] = useState<EmbedPoint | null>(null);

  // Search state
  const [inputValue, setInputValue] = useState("");
  const [activeQuery, setActiveQuery] = useState("");
  const [results, setResults] = useState<ScoredPoint[]>([]);

  // Scatter zoom/fit signals
  const [fitSignal, setFitSignal] = useState(0);
  const [zoomDelta, setZoomDelta] = useState<number>(0);
  const handleFitView = useCallback(() => setFitSignal((n) => n + 1), []);
  const handleZoomHandled = useCallback(() => setZoomDelta(0), []);

  const points = embedData?.[embedType] ?? [];
  const candidateCount = points.filter((p) => p.is_candidate).length;
  const typesPresent = new Set(points.map((p) => p.node_type));

  const hasResults = results.length > 0;
  const highlightedIds = hasResults
    ? new Set(results.map((r) => r.node_id))
    : null;

  const handleSearch = useCallback(
    (query: string) => {
      const found = mockSearch(query, points);
      setActiveQuery(query);
      setResults(found);
      // Auto-fit so highlighted nodes are in view
      setFitSignal((n) => n + 1);
    },
    [points],
  );

  const handleClear = useCallback(() => {
    setInputValue("");
    setActiveQuery("");
    setResults([]);
    setSelectedNode(null);
  }, []);

  const handleSelectResult = useCallback((p: EmbedPoint) => {
    setSelectedNode(p);
  }, []);

  return (
    <div
      style={{
        display: "flex",
        flexDirection: "column",
        height: "100vh",
        overflow: "hidden",
        background: "#0A0909",
      }}
    >
      <RunContextBar
        runId={runId}
        runName={runDetail?.display_name ?? "Run"}
        videoUrl={runDetail?.source_url ?? ""}
        currentPhase={runDetail?.current_phase ?? 2}
        completedPhases={runDetail?.completed_phases ?? 0}
      />

      <div style={{ flex: 1, position: "relative", overflow: "hidden" }}>
        {isLoading ? (
          <LoadingState />
        ) : points.length === 0 ? (
          <EmptyState />
        ) : (
          <>
            {/* Scatter — shrinks right when inspect panel open, top padded for search bar */}
            <div
              style={{
                position: "absolute",
                top: 0,
                left: 0,
                right: selectedNode ? 348 : 0,
                bottom: hasResults ? PANEL_H : 0,
                transition: "right 200ms ease-in-out, bottom 240ms cubic-bezier(0.16,1,0.3,1)",
              }}
            >
              <EmbedScatter
                points={points}
                selectedId={selectedNode?.node_id ?? null}
                onSelect={setSelectedNode}
                fitSignal={fitSignal}
                zoomDelta={zoomDelta}
                onZoomHandled={handleZoomHandled}
                highlightedIds={highlightedIds}
              />
            </div>

            {/* Overlay layer — pointer-events:none passthrough, z-index above scatter */}
            <div
              style={{
                position: "absolute",
                inset: 0,
                zIndex: 20,
                pointerEvents: "none",
              }}
            >
              {/* Search bar */}
              <SearchBar
                value={inputValue}
                onChange={setInputValue}
                onSearch={handleSearch}
                onClear={handleClear}
                hasResults={hasResults}
              />

              {/* Embed type toggle + zoom controls — top right */}
              <div
                style={{
                  position: "absolute",
                  top: 20,
                  right: selectedNode ? 364 : 16,
                  display: "flex",
                  gap: 6,
                  alignItems: "center",
                  pointerEvents: "auto",
                  transition: "right 200ms ease-in-out",
                }}
              >
                {/* Semantic / Multimodal pill */}
                <div
                  style={{
                    display: "flex",
                    background: "rgba(18,16,16,0.9)",
                    backdropFilter: "blur(10px)",
                    border: "1px solid rgba(255,255,255,0.09)",
                    borderRadius: 8,
                    padding: 3,
                    gap: 2,
                  }}
                >
                  {(["semantic", "multimodal"] as EmbedType[]).map((t) => (
                    <button
                      key={t}
                      onClick={() => { setEmbedType(t); setFitSignal((n) => n + 1); }}
                      style={{
                        fontFamily: "'Geist Mono', monospace",
                        fontSize: 11,
                        padding: "4px 10px",
                        borderRadius: 5,
                        border: "none",
                        cursor: "pointer",
                        background: embedType === t ? "rgba(167,139,250,0.18)" : "transparent",
                        color: embedType === t ? "#A78BFA" : "rgba(255,255,255,0.35)",
                        transition: "all 120ms",
                      }}
                    >
                      {t}
                    </button>
                  ))}
                </div>

                {/* Zoom / fit controls */}
                <div
                  style={{
                    display: "flex",
                    background: "rgba(18,16,16,0.9)",
                    backdropFilter: "blur(10px)",
                    border: "1px solid rgba(255,255,255,0.09)",
                    borderRadius: 8,
                    padding: 3,
                    gap: 2,
                  }}
                >
                  {[
                    { label: "+", onClick: () => setZoomDelta(0.25), title: "Zoom in" },
                    { label: "−", onClick: () => setZoomDelta(-0.25), title: "Zoom out" },
                    { label: "⊡", onClick: handleFitView, title: "Fit to view" },
                  ].map(({ label, onClick, title }) => (
                    <button
                      key={label}
                      onClick={onClick}
                      title={title}
                      style={{
                        fontFamily: "'Geist Mono', monospace",
                        fontSize: 13,
                        width: 28,
                        height: 28,
                        display: "flex",
                        alignItems: "center",
                        justifyContent: "center",
                        borderRadius: 5,
                        border: "none",
                        cursor: "pointer",
                        background: "transparent",
                        color: "rgba(255,255,255,0.45)",
                        transition: "all 120ms",
                      }}
                      onMouseEnter={(e) => {
                        e.currentTarget.style.background = "rgba(255,255,255,0.08)";
                        e.currentTarget.style.color = "rgba(255,255,255,0.8)";
                      }}
                      onMouseLeave={(e) => {
                        e.currentTarget.style.background = "transparent";
                        e.currentTarget.style.color = "rgba(255,255,255,0.45)";
                      }}
                    >
                      {label}
                    </button>
                  ))}
                </div>
              </div>

              {/* Legend — bottom left, shifts up when results panel open */}
              <div
                style={{
                  position: "absolute",
                  bottom: hasResults ? PANEL_H + 16 : 16,
                  left: 16,
                  pointerEvents: "auto",
                  background: "rgba(10,9,9,0.82)",
                  backdropFilter: "blur(6px)",
                  border: "1px solid rgba(255,255,255,0.07)",
                  borderRadius: 8,
                  padding: "8px 12px",
                  display: "flex",
                  flexDirection: "column",
                  gap: 5,
                  transition: "bottom 240ms cubic-bezier(0.16,1,0.3,1)",
                }}
              >
                {LEGEND_TYPES.filter((l) => typesPresent.has(l.type)).map((l) => (
                  <div key={l.type} style={{ display: "flex", alignItems: "center", gap: 7 }}>
                    <span
                      style={{
                        width: 8,
                        height: 8,
                        borderRadius: "50%",
                        background: l.color,
                        flexShrink: 0,
                        opacity: highlightedIds
                          ? [...highlightedIds].some(
                              (id) => points.find((p) => p.node_id === id)?.node_type === l.type
                            )
                            ? 1
                            : 0.25
                          : 1,
                        transition: "opacity 200ms",
                      }}
                    />
                    <span
                      style={{
                        fontFamily: "'Geist Mono', monospace",
                        fontSize: 10,
                        color: "rgba(255,255,255,0.5)",
                        transition: "opacity 200ms",
                        opacity: highlightedIds
                          ? [...highlightedIds].some(
                              (id) => points.find((p) => p.node_id === id)?.node_type === l.type
                            )
                            ? 1
                            : 0.3
                          : 1,
                      }}
                    >
                      {l.type}
                    </span>
                  </div>
                ))}
                {candidateCount > 0 && (
                  <div
                    style={{
                      borderTop: "1px solid rgba(255,255,255,0.07)",
                      marginTop: 3,
                      paddingTop: 5,
                      display: "flex",
                      alignItems: "center",
                      gap: 7,
                    }}
                  >
                    <span
                      style={{
                        width: 8,
                        height: 8,
                        borderRadius: "50%",
                        border: "1px solid rgba(251,178,73,0.6)",
                        background: "transparent",
                        flexShrink: 0,
                      }}
                    />
                    <span
                      style={{
                        fontFamily: "'Geist Mono', monospace",
                        fontSize: 10,
                        color: "rgba(251,178,73,0.6)",
                      }}
                    >
                      clip candidate
                    </span>
                  </div>
                )}
              </div>

              {/* Node count chip — bottom right */}
              <div
                style={{
                  position: "absolute",
                  bottom: hasResults ? PANEL_H + 16 : 16,
                  right: selectedNode ? 364 : 16,
                  pointerEvents: "none",
                  fontFamily: "'Geist Mono', monospace",
                  fontSize: 10,
                  color: "rgba(255,255,255,0.2)",
                  transition: "bottom 240ms cubic-bezier(0.16,1,0.3,1), right 200ms ease-in-out",
                }}
              >
                {points.length} nodes · {candidateCount} candidates
              </div>

              {/* Search results panel */}
              <SearchResultsPanel
                query={activeQuery}
                results={results}
                selectedId={selectedNode?.node_id ?? null}
                onSelect={handleSelectResult}
                onClose={handleClear}
              />

              {/* Inspect panel */}
              <EmbedInspectPanel
                node={selectedNode}
                runId={runId}
                onClose={() => setSelectedNode(null)}
              />
            </div>
          </>
        )}
      </div>
    </div>
  );
}

function LoadingState() {
  return (
    <div
      style={{
        position: "absolute",
        inset: 0,
        display: "flex",
        flexDirection: "column",
        alignItems: "center",
        justifyContent: "center",
        gap: 12,
      }}
    >
      <div
        style={{
          width: 32,
          height: 32,
          border: "2px solid var(--color-border)",
          borderTopColor: "var(--color-violet)",
          borderRadius: "50%",
          animation: "spin 0.8s linear infinite",
        }}
      />
      <span
        style={{
          fontFamily: "'Geist Mono', monospace",
          fontSize: 12,
          color: "var(--color-text-muted)",
        }}
      >
        Loading…
      </span>
    </div>
  );
}

function EmptyState() {
  return (
    <div
      style={{
        position: "absolute",
        inset: 0,
        display: "flex",
        flexDirection: "column",
        alignItems: "center",
        justifyContent: "center",
        gap: 14,
      }}
    >
      <div
        style={{
          width: 44,
          height: 44,
          borderRadius: "50%",
          background: "rgba(167,139,250,0.1)",
          border: "1px solid rgba(167,139,250,0.2)",
          display: "flex",
          alignItems: "center",
          justifyContent: "center",
        }}
      >
        <Sparkles size={20} color="#A78BFA" />
      </div>
      <div style={{ textAlign: "center" }}>
        <p
          style={{
            fontFamily: "'Bricolage Grotesque', sans-serif",
            fontWeight: 600,
            fontSize: 15,
            color: "var(--color-text-primary)",
            margin: "0 0 6px 0",
          }}
        >
          Embeddings not yet available
        </p>
        <p
          style={{
            fontFamily: "'Plus Jakarta Sans', sans-serif",
            fontSize: 13,
            color: "var(--color-text-muted)",
            margin: 0,
          }}
        >
          The semantic search space will appear once Phase 2 completes.
        </p>
      </div>
    </div>
  );
}
